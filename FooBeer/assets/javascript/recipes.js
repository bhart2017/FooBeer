...
function createFoodBeerPairings(){
...
//create loops that will pull the title, picture, recipe, link to instructions and post to page
    	...
    	for (var i=0; i<recipes.length; i++){
            ...
            var foodBeerPairingsContainer = $("<div>").attr("class", "foodBeerPairings")
            $(".recipes").append(foodBeerPairingsContainer)
        ...
        //if we have beers for the user, input show on the screen
        if (userInput in foodBeerPairings){
            var beers = foodBeerPairings[userInput]

            var ulHtml = $("<ul>").addClass("bold").text("Beers to accompany your meal:")
            for (var i=0; i<beers.length; i++){
                var liHtml = $("<li>").text(beers[i])
                ulHtml.append(liHtml)

            }
            $(".foodBeerPairings").append(ulHtml)
